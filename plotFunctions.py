'''
	Nice functions for performing plotting, making use of seaborn
'''
...
def plot_distribution(data, column, bins=50, correlation=None):
    ...
    if not correlation is None:
        ...
        column = column + ' - ' + str(round(value,2))
    plt.title('Distribution of {0}'.format(column), fontsize=18)
    plt.grid(False)
...
def plot_bivariate(data, x, y, hue=None, **kwargs):
    ...
    if hue:
        plt.title('{0} vs {1}, by {2}'.format(x,y,hue), fontsize=18)
    else:
        plt.title('{0} vs {1}'.format(x,y), fontsize=18)
...
def segm_target(data, cat, target):
    ...
    We use groupby to split the target distribution across different
    categories
    '''
